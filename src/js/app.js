    ...
    finishDrag() {
        if (this.draggedCard && this.targetColumn !== null) {
            ...
            if (cards.length === 0 || this.targetPosition >= cards.length) {
                const newCard = this.app.createCardElement(cardContent, cardId);
...
export class TrelloApp {
    ...
    generateCardIds() {
        ['todo', 'inProgress', 'done'].forEach((columnId) => {
            this.columns[columnId] = this.columns[columnId].map(text => ({
                id: `card_${Date.now()}_${this.cardCounter++}`,
                text: text
            }));
        });
    }
    ...
    renderBoard() {
        ['todo', 'inProgress', 'done'].forEach((columnId) => {
            ...
            this.columns[columnId].forEach((cardData) => {
                const cardElement = this.createCardElement(cardData.text, cardData.id);
                container.appendChild(cardElement);
            });
        });
    }

    createCardElement(text, id = null) {
        const card = document.createElement('div');
        card.className = 'card';
        card.draggable = true;
        
        const cardId = id || `card_${Date.now()}_${this.cardCounter++}`;
    ...
    deleteCard(cardElement) {
        ...
        this.columns[column] = this.columns[column].filter((cardData) => cardData.id !== cardId);
    ...
    addCardToColumn(columnId, text, save = true) {
        ...
        this.columns[columnId].push(cardData);
        const container = document.querySelector(`[data-column="${columnId}"] .cards-container`);
        const cardElement = this.createCardElement(text, cardData.id);
        container.appendChild(cardElement);
    ...
    updateColumnData() {
        ['todo', 'inProgress', 'done'].forEach((columnId) => {
            ...
            this.columns[columnId] = cards.map((card) => ({
                id: card.dataset.cardId,
                text: card.querySelector('.card-content').textContent
            }));
        });
    ...
    saveAndUpdate() {
        const dataForStorage = {
            todo: this.columns.todo.map(card => card.text),
            inProgress: this.columns.inProgress.map(card => card.text),
            done: this.columns.done.map(card => card.text)
        };
        
        Storage.saveBoardData(dataForStorage);
        this.updateCardCounts();
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('5. Each card has unique ID for proper deletion');
});